...
;(function($, window, document, undefined) { // 将window/document等系统变量作为参数传递到插件内部，这样在需要访问这些系统变量的时候就可以访问这些参数，也就是访问局部变量，比访问全局变量速度更快，有少许性能提升
	...
	Tab.prototype = {
		...
		_autoPlay: function() {
			var tabs = this.tabs;
			// 在原型方法内部把this负值给that，而不是在原型属性里，
			// 如果在原型属性里赋值而在外层函数里没有把this负值给that，在定时器内部还是访问不到的，
			// 因为闭包里的this还是指向全局作用域（这里用到了定时器，定时器内this指向window对象），
			// 而在外层函数里把this负值给that，闭包中的匿名函数就能访问到了；这里的this在调用时就是Tab的实例
			var that = this; 
